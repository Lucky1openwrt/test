...
void verify_valid_hex_str(char* s) {
	...
	if (strncmp(s,"-",1) == 0) { // Don't check -switch
		s = "00" ; // dummy to pass
	}
...
void verify_args(int argc, char** argv) {
	...
	if (argc < 3) {
		printf("Usage: %s <firmware file> -v <version hash> <signing hash1> ... <signing hash n>\n", argv[0]);
		exit(1);
	}
...
void sign_firmware(char* filename, char** salts, int num_salts) {
	...
	if (strcmp(salts[0],"-v") == 0) {
		init_salt(&salt, salts[1]);
		version_suffix = convert_hex_to_bin(salts[1]);
		i = 2; // skip switch & value
		}
	
	
	printf("versoin suffix: %s \n", version_suffix);
